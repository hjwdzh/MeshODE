...
struct BarycentricDistanceLoss {
  ...
  template <typename T>
  bool operator()(const T* const p1, const T* const p2, const T* const p3, T* residuals) const {
    for (int j = 0; j < 3; ++j) {
      residuals[j] = (T)w[0] * p1[j] + (T)w[1] * p2[j] + (T)w[2] * p3[j] - (T)tar[j];
    }
    return true;
  }

  static ceres::CostFunction* Create(const Vector3& w_, const Vector3& tar_) {
     return (new ceres::AutoDiffCostFunction<BarycentricDistanceLoss, 3, 3, 3, 3>(
...
struct PointRegularizerLoss {
  ...
  static ceres::CostFunction* Create(const double lambda_, const Vector3& tar_) {
