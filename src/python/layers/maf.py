...
class MADE(nn.Module):
    ...
    def inverse(self, u, y=None, sum_log_abs_det_jacobians=None):
        ...
        for i in self.input_degrees:
            m, loga = self.net(self.net_input(x, y)).chunk(chunks=2, dim=1)
            slice_a = x[:,:i]
            slice_b = u[:,i:i+1] * torch.exp(loga[:,i:i+1]) + m[:,i:i+1]
            slice_c = x[:,i+1:]
            x = torch.cat((slice_a, slice_b, slice_c),1)
...
if __name__ == '__main__':
    ...
    model = MAF(5, 3, 256, 1, None,
                'relu', 'sequential', batch_norm=False)

    optimizer = optim.Adam(model.parameters(), lr=1e-3)
    optimizer.zero_grad()

    pp = torch.zeros(1,50)
    qq = model.forward(pp)[0]
    cc = model.inverse(qq)[0]
    print((pp-cc).abs().max(), (pp-qq).abs().max())
    loss = qq.sum()
    loss.backward()
    optimizer.step()
